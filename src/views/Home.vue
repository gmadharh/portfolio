<template>
  <main
    class="flex min-h-screen flex-col items-center justify-center px-6 py-12 transition-colors duration-300"
  >
    <!-- Theme Toggle -->
    <button
      @click="toggleTheme"
      class="fixed right-4 top-4 p-3 opacity-40 transition-opacity duration-300 hover:opacity-100 md:right-6 md:top-6"
    ...
    <div class="w-full max-w-lg space-y-12">
      ...
      <header class="space-y-4">
        <h1 class="font-display text-4xl uppercase tracking-wider md:text-5xl">
          <span :class="{ cursor: !nameComplete }">{{ displayedName }}</span>
        </h1>
        <p
          v-if="showSubtitle"
          class="animate-fade-in font-sans text-base tracking-wide opacity-0 md:text-lg"
        >
          I am <span class="cursor">{{ displayedTrait }}</span>
        </p>
        <div class="h-px w-16 bg-current opacity-40"></div>
      </header>

      <!-- Bio -->
      <section class="stagger-2 animate-fade-up space-y-4 opacity-0">
        <p class="font-serif text-sm leading-relaxed opacity-80 md:text-base">
          Software developer. I view tech as leverage to solve problems and
          build real value.<br />
          Documenting the journey at the intersection of tech, wealth, and
          faith.
        </p>
      </section>
      ...
      <!-- Social Links -->
      <nav
        class="stagger-4 flex animate-fade-up flex-wrap gap-5 opacity-0 md:gap-6"
      >
        <!-- YouTube -->
        <a
      ...
      <!-- Footer -->
      <footer class="stagger-6 animate-fade-up pt-8 opacity-0">
